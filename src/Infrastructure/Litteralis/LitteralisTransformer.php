...
final readonly class LitteralisTransformer
{
    ...
    public function transform(
        ...
        $generalInfoCommand->identifier = $properties['arretesrcid'];
        $this->setCategory($generalInfoCommand, $properties, $reporter);
    ...
    private function setCategory(SaveRegulationGeneralInfoCommand $generalInfoCommand, array $properties, LitteralisReporter $reporter): void
    {
        $documentTypeValue = $properties['documenttype'];
        $isPermanentByDocumentType = $documentTypeValue === 'ARRETE PERMANENT';
        ...
        $isPermanentByCategory = $generalInfoCommand->category === RegulationOrderCategoryEnum::PERMANENT_REGULATION->value;

        // Check that the "permanent" status is consistent between 'documenttype' and 'categoriesmodele'
        if ($isPermanentByCategory !== $isPermanentByDocumentType) {
            $reporter->addWarning($reporter::WARNING_PERMANENT_CATEGORY_MISMATCH, [
                'arretesrcid' => $properties['arretesrcid'],
                'documenttype' => $documentTypeValue,
                'categoriesmodele' => $categoriesModeleValue,
            ]);
        }
    ...
    private function gatherMeasureParameters(array $properties, LitteralisReporter $reporter): array
    {
        ...
        $allMeasureNames = explode(';', $properties['mesures']);
        ...
        foreach ($allMeasureNames as $name) {
            if (!\array_key_exists($name, self::MEASURE_MAP)) {
                $reporter->addNotice($reporter::NOTICE_UNSUPPORTED_MEASURE, ['name' => $name]);
                continue;
            }
        ...
        foreach ($measureParameters as $params) {
            // Exemple : "Circulation interdite 2 | dérogation : urgences" -> ['Circulation interdite 2', 'dérogation : urgences']
            [$name, $item] = $this->parseSeparatedString($params, '|');

            if (preg_match('/^(?P<name>\w+) (?P<number>\d+)$/i', $name, $matches)) {
                ...
                $index = array_search($cleanedName, $measureNames);

                if ($index === false) {
                    // Ce paramètre ne correspond à aucune mesure.
                    // C'est probablement un type de mesure qu'on n'importe pas.
                ...
                $index = array_search($name, $measureNames);
    ...
    private function parseMaxSpeed(array $properties, array $parameters, LitteralisReporter $reporter): ?int
    {
        ...
        if (!$value) {
            $reporter->addError($reporter::ERROR_MAX_SPEED_VALUE_MISSING, ['idemprise' => $properties['idemprise']]);

            return null;
        }

        if (!preg_match('/^(?P<speed>\d+)/i', $value, $matches)) {
            $reporter->addError($reporter::ERROR_MAX_SPEED_PARSING_FAILED, ['idemprise' => $properties['idemprise'], 'limite de vitesse' => $value]);
