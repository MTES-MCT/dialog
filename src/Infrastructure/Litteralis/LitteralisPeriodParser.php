...
final class LitteralisPeriodParser
{
    ...
    private const HOURS_REGEX = '/^(?:la nuit |de nuit )?(?:de )?(?P<startHour>\d{1,2}) ?(?:h|heures?) ?(?P<startMinute>\d{1,2})? à ?(?P<endHour>\d{1,2}) ?(?:h|heures?) ?(?P<endMinute>\d{1,2})?\.?$/i';
    ...
    public static function parse(string $value, ?\DateTimeZone $tz = null): ?array
    {
        ...
        if (preg_match(self::HOURS_REGEX, $value, $matches)) {
            ...
            $startMinute = $matches['startMinutes'] ?? '00';
            $endHour = $matches['endHour'];
            $endMinute = $matches['endMinutes'] ?? '00';
