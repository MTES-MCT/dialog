...
final class LitteralisReportFormatter
{
    ...
    private function makeTranslationContext(array $context): array
    {
        $translationContext = [];

        foreach ($context as $key => $value) {
            if (str_starts_with('%', $key) && str_ends_with('%', $key)) {
                $translationContext[$key] = $value;
            }
        }
    ...
    public function format(array $records): string
    {
        ...
        foreach ($this->findRecordsByType(LitteralisReporter::FACT, $records) as [$name, $context]) {
            $verboseName = $this->translator->trans(\sprintf('litteralis.report.fact.%s', $name), $this->makeTranslationContext($context));
            $value = $context['value'];
        ...
        foreach ($this->findRecordsByType(LitteralisReporter::COUNT, $records) as [$name, $context]) {
            $verboseName = $this->translator->trans(\sprintf('litteralis.report.count.%s', $name), $this->makeTranslationContext($context));
            $value = $context['value'];
            $lines[] = \sprintf('%s : %s', $verboseName, $value);
        }
