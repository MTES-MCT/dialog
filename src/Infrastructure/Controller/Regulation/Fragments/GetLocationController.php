...
final class GetLocationController extends AbstractRegulationController
{
    ...
    #[Route(
        '/_fragment/regulations/{regulationOrderUuid}/location/{uuid}',
        name: 'fragment_regulations_location',
        methods: ['GET'],
        requirements: [
            'regulationOrderUuid' => '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}',
            'uuid' => '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}',
        ],
    )]
    public function __invoke(string $regulationOrderUuid, string $uuid): Response
    {
        $regulationOrderRecord = $this->getRegulationOrderRecord($regulationOrderUuid);
            ...
            $this->twig->render(
                ...
                context: [
                    'location' => new DetailLocationView(
                        $location->getUuid(),
                        LocationAddress::fromString($location->getAddress()),
                        $location->getFromHouseNumber(),
                        $location->getToHouseNumber(),
                    ),
