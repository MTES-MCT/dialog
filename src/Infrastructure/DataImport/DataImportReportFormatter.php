...
final class DataImportReportFormatter
{
    ...
    public function format(array $records): string
    {
        ...
        $lines[] = $this->header('=', $this->translator->trans(
            'dataimport.report.title',
            [
                '%entity%' => 'Eudonet Paris',
            ],
        ),
        );
        $lines[] = '';
        ...
        $lines[] = $this->header('-', $this->translator->trans('dataimport.report.section.fact.title'));
        $lines[] = '';

        foreach ($this->findRecordsByType(DataImportReporter::FACT, $records) as [$name, $context]) {
            $verboseName = $this->translator->trans(\sprintf('dataimport.report.fact.%s', $name));
            $value = $context['value'];
        ...
        $lines[] = $this->header('-', $this->translator->trans('dataimport.report.section.count.title'));
        $lines[] = '';

        foreach ($this->findRecordsByType(DataImportReporter::COUNT, $records) as [$name, $context]) {
            $verboseName = $this->translator->trans(
                \sprintf('dataimport.report.count.%s', $name),
                [
                    '%entity%' => 'Eudonet Paris',
                ],
            );
            ...
            if (\array_key_exists('regulationsCount', $context)) {
                $line = \sprintf(
                    ...
                    $this->translator->trans(
                        'dataimport.report.among_regulations',
                        [
        ...
        $caseLists = [
            DataImportReporter::ERROR => [],
            DataImportReporter::WARNING => [],
            DataImportReporter::NOTICE => [],
        ];
        ...
        foreach ($records as [$type, $context]) {
            ...
            if (\array_key_exists('regulation_id', $context)) {
                $regulationId = $context['regulation_id'];
        ...
        foreach ($caseLists as $type => $cases) {
            $lines[] = $this->header('-', $this->translator->trans(\sprintf('dataimport.report.section.%s.title', $type)));
            $lines[] = '';

            foreach ($cases as $name => $info) {
                // Affichage du cas et du nombre d'occurrences
                $verboseName = $this->translator->trans(
                    \sprintf('dataimport.report.%s.%s', $type, $name),
                    [
                        '%entity%' => 'Eudonet Paris',
                    ],
                );
                ...
                $lines[] = \sprintf('  %s :', $this->translator->trans('dataimport.report.regulations'));
