...
Cas d’erreur métier (géocodage / périmètre géographique) — même statut 422, sans tableau de violations:
...
Remarques:
- Les erreurs métier possibles incluent notamment: échec de géocodage de voie, abscisse hors plage, échec de géocodage de route numérotée, impossibilité d’intervention de l’organisation sur la géométrie.
- L’opération de création est atomique: si la mesure échoue (validation ou géocodage), aucune information générale n’est persistée.
