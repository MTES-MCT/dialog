...
final class MapControllerTest extends AbstractWebTestCase
{
    ...
    public function testGetAuthenticatedRestoresPublishedWhenOpeningAnonymousLink(): void
    {
        // A link generated by an anonymous visitor carries filter params but never the
        // "Statut des arrêtés" marker. A logged-in user opening it must still see published
        // arrêtés: the "published" checkbox has to stay checked instead of being reset by the
        // absent checkbox.
        $client = $this->login();
        $crawler = $client->request('GET', '/carte?' . http_build_query([
            'map_filter_form' => [
                ...
                'displayTemporaryRegulations' => 'yes',
                'measureTypes' => ['noEntry'],
            ],
        ]));

        $this->assertResponseStatusCodeSame(200);

        $this->assertSame('checked', $crawler->filter('[name="map_filter_form[displayPublished]"]')->attr('checked'));
        $this->assertNull($crawler->filter('[name="map_filter_form[displayDrafts]"]')->attr('checked'));
    }

    public function testGetAuthenticatedKeepsPublishedUncheckedWhenExplicitlyUnset(): void
    {
        // When the status filter marker is present (link generated by a logged-in user) the
        // absent "published" checkbox is a deliberate choice and must be honoured.
        $client = $this->login();
        $crawler = $client->request('GET', '/carte?' . http_build_query([
            'map_filter_form' => [
                'measureTypes' => ['noEntry'],
        ...
        $this->assertNull($crawler->filter('[name="map_filter_form[displayPublished]"]')->attr('checked'));
        $this->assertSame('checked', $crawler->filter('[name="map_filter_form[displayDrafts]"]')->attr('checked'));
    }
