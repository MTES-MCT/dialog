...
final class ListRegulationsControllerTest extends AbstractWebTestCase
{
    ...
    public function testSortChevronsAreRendered(): void
    {
        ...
        // Identifiant, Période, Source, Statut → 4 colonnes triables
        $chevrons = $crawler->filter('[data-testid="app-regulation-table"] thead .fr-btn--sort');
        $this->assertSame(4, $chevrons->count());
        $this->assertSame(4, $crawler->filter('[data-testid="app-regulation-table"] thead th[aria-sort]')->count());
    }
    ...
    public function testPublishedRegulationRendering(): void
    {
        ...
        $this->assertSame('Modifier', $links->eq(1)->text());
        $this->assertSame('http://localhost/regulations/' . RegulationOrderRecordFixture::UUID_PUBLISHED, $links->eq(1)->link()->getUri());
    }
