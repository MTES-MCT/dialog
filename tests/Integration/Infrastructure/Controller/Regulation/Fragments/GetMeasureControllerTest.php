...
final class GetMeasureControllerTest extends AbstractWebTestCase
{
    public function testGet(): void
    {
        ...
        $this->assertSame('Rue Adrien Lesesne à Saint-Ouen-sur-Seine Afficher la carte Masquer la carte Aperçu de la localisation', $measure1Content->filter('li')->eq(3)->text());
        $this->assertSame(LocationFixture::UUID_TYPICAL, $measure1Content->filter('li')->eq(4)->attr('data-location-uuid'));
        $this->assertSame('Rue Eugène Berthoud du n° 47 au n° 65 à Saint-Ouen-sur-Seine Afficher la carte Masquer la carte Aperçu de la localisation', $measure1Content->filter('.app-card__content li')->eq(4)->text());
    ...
    public function testGetWithComplexVehicles(): void
    {
        ...
        $this->assertSame('Rue Ardoin du n° 87 au n° 63 à Saint-Ouen-sur-Seine Afficher la carte Masquer la carte Aperçu de la localisation', $measure1Content->filter('li')->eq(3)->text());
        $this->assertSame('Rue La Clef Des Champs à Saint-Ouen-sur-Seine Afficher la carte Masquer la carte Aperçu de la localisation', $measure1Content->filter('li')->eq(4)->text());
        $this->assertSame('D322 (Ardennes) du PR 1+0 (côté U) au PR 4+0 (côté U) Afficher la carte Masquer la carte Aperçu de la localisation', $measure1Content->filter('li')->eq(5)->text());
        $this->assertSame('Rue Albert Dhalenne du n° 12 au n° 34 à Saint-Ouen-sur-Seine Afficher la carte Masquer la carte Aperçu de la localisation', $measure1Content->filter('li')->eq(6)->text());
    }
