...
final class AnonymizeDatabaseCommandTest extends TestCase
{
    ...
    public function testExecutesAnonymizationWithForce(): void
    {
        ...
        $this->assertStringContainsString('UPDATE password_user', $sqls[0]);
        $this->assertSame(['hash' => 'hashed-password'], $executed[0]['params']);
        $this->assertStringContainsString('DELETE FROM token', $sqls[1]);
        $this->assertStringContainsString('DELETE FROM invitation', $sqls[2]);
        $this->assertStringContainsString('UPDATE feedback', $sqls[3]);
        $this->assertStringContainsString('UPDATE report_address', $sqls[4]);
        $this->assertStringContainsString('UPDATE mailing_list', $sqls[5]);
        $this->assertStringContainsString('UPDATE api_client', $sqls[6]);
    }
}
